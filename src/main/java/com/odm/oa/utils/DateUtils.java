...
public class DateUtils {
	...
	public String gerUnixTime2String(String timestampString, String formats) {
		if (timestampString != null && timestampString != "") {// 默认格式化形式
			return "";
		}
	 ...
	 * @return
	 */
	public static Date formatType(String dateStr, String format) {
		if (dateStr == null || dateStr == "") {
			return new Date();
		}
	...
	public static List<String> getBetweenDates(String startDate, String endDate) {
		...
		Date d1 = parseDate(startDate);//定义起始日期
		Date d2 = parseDate(endDate);//定义结束日期
		...
		while(dd1.compareTo(dd2) <= 0){//判断是否到结束日期
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			String str = sdf.format(dd1.getTime());
			dates.add(str);
			dd1.add(Calendar.DAY_OF_MONTH, 1);//进行当前日期月份加1
		}
		return dates;
	}
